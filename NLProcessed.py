...
class NLProcessed:
    ...
    def load_text(self, filename, label=None, parser=None):
        ...
        Args:
            self (NLProcessed): class objecgt
        ...
        if parser is None:
            try:
                # parse the results
                results = NLProcessed._default_parser(filename)
    ...
    def sentence_sentiment(self, columns=5, rows=2, hist=True):
        ...
        if hist:
            fig, axs = plt.subplots(rows, columns, figsize=(columns, rows), sharey=True,
                                    num='SENTIMENT SUBPLOTS - DENSITY')
        else:
            fig, axs = plt.subplots(rows, columns, figsize=(columns, rows), sharey=True, num='SENTIMENT SUBPLOTS - KDE')
        ...
        for key, value in self.data['sent_sent'].items():
            ...
            if hist:
                ...
                sns.kdeplot(x=polarities, y=subjectivities, color="#482b7a")
